...
.sheet{width:210mm;min-height:297mm;margin:16px auto;background:#fff;padding:0;box-shadow:0 10px 30px #0002;page-break-after:always}
...
@media print{body{background:#fff}.toolbar{display:none!important}.sheet{margin:0;box-shadow:none;width:auto;min-height:0;height:auto;padding:0;page-break-inside:avoid}}`;
}
...
function openSalesPrintWindow(title,bodyHtml,opts={}){
  ...
  const html=`<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title></title><style>${salesPrintCss()}</style></head><body>
    <div class="toolbar"><b>${salesEsc(title)}</b><button class="primary" type="button" id="btnPrint">Yazdır / PDF Kaydet</button>${wa}<button type="button" onclick="window.close()">Kapat</button></div>
    ${bodyHtml}
    <script>
    (function(){
