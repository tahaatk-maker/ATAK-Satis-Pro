...
function goTab(id,{remember=true,financeJump=''}={}){
  if(id==='uninvoicedSales'){id='financeCenter';financeJump=financeJump||'uninvoiced'}
  ...
  qa('[data-tab]').forEach(x=>{
    const same=x.dataset.tab===id;
    const jumpMatch=!financeJump || x.dataset.financeJump===financeJump || (!x.dataset.financeJump && !financeJump);
    x.classList.toggle('active',same && (id!=='financeCenter' || jumpMatch || (!x.dataset.financeJump && financeJump!=='uninvoiced')));
  });
  // Finans alt menü: Kesilmeyen seçiliyse o butonu aktif tut
  if(id==='financeCenter'&&financeJump==='uninvoiced'){
    qa('[data-tab="financeCenter"]').forEach(x=>x.classList.toggle('active',x.dataset.financeJump==='uninvoiced'));
  }else if(id==='financeCenter'){
    qa('[data-tab="financeCenter"]').forEach(x=>x.classList.toggle('active',!x.dataset.financeJump));
  }
  qa('.tab').forEach(x=>x.classList.toggle('active',x.id===id));
  const btn=financeJump?q(`[data-tab="${id}"][data-finance-jump="${financeJump}"]`):q(`[data-tab="${id}"]:not([data-finance-jump])`)||q(`[data-tab="${id}"]`);
  ...
  if(id==='financeCenter')setTimeout(async()=>{
    try{await loadFinanceCenter()}catch(e){toast(e.message)}
    if(financeJump==='uninvoiced'){
      q('#financeUninvoicedPanel')?.scrollIntoView({behavior:'smooth',block:'start'});
      q('#financeUninvoicedPanel')?.classList.add('pulse-highlight');
      setTimeout(()=>q('#financeUninvoicedPanel')?.classList.remove('pulse-highlight'),1800);
    }
  },20);
...
async function renderDashboardRevenue(){
  try{
    ...
    try{
      ...
      // Eski sunucu: POS satışlarını finance-center'dan say
      try{
        const fin=await api('/web-api/admin/finance-center?customers=0');
        const sales=(fin.sales||fin.transactions||[]).filter(t=>t.kind==='sale'&&!t.cancelled&&String(t.date||'').slice(0,10)===d);
        netToday=sales.reduce((a,t)=>a+(Number(t.total)!=null&&t.total!==''?Number(t.total):Math.abs(Number(t.customerDelta||0))),0);
        netCount=sales.length;
        primToday=sales.reduce((a,t)=>a+Number(t.commissionAmount||0),0);
      }catch(__){}
    }
...
async function loadSalesCenter(){
  try{
    // Müşteri listesini finance-center'dan al (customers/search eski sunucuda yok → 302)
    ...
    // Küçük cari: doğrudan kullan; büyükse ara ile
    salesCenterData._customerFallback=fromFin.length?fromFin:fromCat;
    salesCenterData.customers=salesCenterData._customerFallback.slice(0,500);
    salesCenterData.customerTotal=Number(fin.customerTotal||salesCenterData._customerFallback.length||0);
    salesCenterData.accounts=fin.accounts||cat.accounts||[];
    ...
    // 500 ve altı: select'i doldur — Ara olmadan da seçilebilsin
    const list=salesCenterData.customers||[];
    if(q('#salesCustomerSelect')){
      if(list.length && list.length<=500){
        q('#salesCustomerSelect').innerHTML='<option value="">Müşteri seçin</option>'+
          list.map(c=>`<option value="${c.id}">${c.name}${c.phone?' · '+c.phone:''}</option>`).join('');
      }else{
        q('#salesCustomerSelect').innerHTML='<option value="">Önce arayın, sonra seçin</option>';
      }
    }
    if(q('#salesCustomerCount'))q('#salesCustomerCount').textContent=(salesCenterData.customerTotal||list.length||0)+' kayıt';
    if(q('#salesCustomerSearchHint'))q('#salesCustomerSearchHint').textContent=
      list.length?`Toplam ${salesCenterData.customerTotal||list.length} müşteri yüklendi. İsim yazarak süzün veya listeden seçin.`
...
async function salesSearchCustomers(){
  ...
  let rows=[],total=0;
  // 1) Yerel liste (finance-center) — eski sunucuda tek güvenilir yol
  ...
  if(term){
    ...
    total=rows.length;
    // 2) Varsa sunucu araması (yeni sürüm)
    if(!rows.length){
      try{
...
@media print{body{background:#fff}.toolbar{display:none!important}.sheet{margin:0;box-shadow:none;width:auto;min-height:auto;padding:0}}`;
}
...
function salesCombinedContractSenetA4Html(d){
  const items=d.items||[];
  const rows=items.slice(0,12).map(i=>`<tr><td>${salesEsc(i.itemCode||i.productCode||'-')}</td><td>${salesEsc(i.materialCode||i.productName||i.productCode)}</td><td class="num">${i.quantity}</td><td class="num">${salesMoney(i.unitPrice)}</td><td class="num">${salesMoney(i.quantity*i.unitPrice)}</td></tr>`).join('');
  const addr=[d.customer?.address,d.customer?.district,d.customer?.city].filter(Boolean).join(', ')||'-';
  const cfg=salesPromissorySettings||{},creditor=cfg.creditorName||'Atak Pazarlama';
  const schedule=d.promissory?.schedule||[];
  const senetTotal=d.promissory?Number(d.promissory.amount||0):0;
  const schedRows=schedule.map((r,idx)=>`<tr><td>${idx+1}</td><td>TASLAK-${String(idx+1).padStart(2,'0')}</td><td>${salesEsc(r.dueDate)}</td><td class="num">${salesMoney(r.amount)}</td></tr>`).join('');
  const pays=(d.payments||[]).map(p=>`${salesEsc(p.method)}: ${salesMoney(p.amount)}`).join(' · ');
  return `<section class="sheet a4-one">
<style>
.a4-one{padding:8mm!important;font-size:11px;line-height:1.35}
.a4-one .doc-head{padding-bottom:6px;margin-bottom:8px}
.a4-one .brand{font-size:16px}.a4-one h2{font-size:13px;margin:0 0 6px}
.a4-one .grid2{gap:6px;margin:6px 0}.a4-one .box{padding:6px 8px}
.a4-one table{margin:6px 0;font-size:10px}th,td{padding:3px 4px!important}
.a4-one .totals{width:260px;font-size:10px}.a4-one .terms{font-size:9px;margin-top:6px}
.a4-one .signs{gap:16px;margin-top:10px}.a4-one .sig{min-height:40px}
.a4-one .split{display:grid;grid-template-columns:1.1fr .9fr;gap:8px;margin-top:8px;border-top:2px solid #0b2a55;padding-top:8px}
.a4-one .senet-box{border:1px solid #0b2a55;border-radius:8px;padding:8px}
.a4-one .senet-box h3{margin:0;font-size:13px;color:#0b2a55}
.a4-one .senet-box .amount{font-size:18px;font-weight:900;color:#0b2a55;margin:4px 0}
.a4-one .senet-box .words,.a4-one .senet-box p{font-size:9px;margin:3px 0}
.a4-one table.sched{width:100%;border-collapse:collapse;font-size:9px;margin-top:4px}
.a4-one table.sched th,.a4-one table.sched td{border-bottom:1px solid #e3ebf4;padding:2px 3px}
.a4-one .num{text-align:right}
@media print{.a4-one{page-break-after:avoid!important;min-height:auto!important}}
</style>
    <div class="doc-head"><div class="brand">ATAK PAZARLAMA<small>Sözleşme + Senet · Tek A4</small></div>
    ...
    ${salesCustomerPrintTotals(d)}
    <div style="font-size:10px;margin-top:4px"><b>Ödeme:</b> ${salesEsc(d.method||'-')}${pays?` · ${pays}`:''}</div>
    <div class="terms"><b>Şartlar</b><ol>
      <li>Ürün bedeli ödeme planına göre tahsil edilir.</li>
      <li>Teslimatta kontrol müşteriye aittir.</li>
      <li>Senetler bu sözleşmenin eki ve ayrılmaz parçasıdır.</li>
      <li>Bu belge mali fatura yerine geçmez.</li>
    </ol></div>
    <div class="split">
      <div class="signs"><div class="sig"><small>Satıcı İmza</small>${salesEsc(creditor)}</div><div class="sig"><small>Müşteri İmza</small>${salesEsc(d.customer?.name||'')}</div></div>
      <div class="senet-box">
        <h3>SENET</h3>
        ${senetTotal>0?`
        ...
        `:`<p>Bu satışta senet tutarı yok. Senet için ödeme satırına Senet girin.</p>`}
      </div>
    </div>
  </section>`;
}
...
async function confirmSalesDraft(){
  ...
  try{
    ...
    closeSalesPreview();status.textContent=`Satış kaydedildi. Yeni cari bakiye: ${salesMoney(result.balance)}${noteText}`;status.className='form-status success';await check();await loadSalesCenter();try{await renderDashboardRevenue()}catch(_){ }salesReset();
...
const INV_VIEW_META={
  ...
  pending_sales:{title:'Kesilmeyen Faturalar',hint:'Geç kesilen satışlar. Manuel kesebilir veya QNB kuyruğuna alabilirsiniz. Finans & Cari’de de vardır.'},
